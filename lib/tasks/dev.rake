namespace :dev do
  ...
  task setup: :environment do
    if Rails.env.development?
      ...
      show_spinner("Cadastrando treinos padrões...") { %x(rails dev:add_trainings) }
  ...
  desc "Adiciona treinos padrões"
  task add_trainings: :environment do
    file_name = 'trainings.txt'
    file_path = File.join(DEFAULT_FILES_PATH, file_name)

    File.open(file_path, 'r').each do |line|
      Training.create!(description: line.strip)
    end
  end
